...
const create =  async (projectObj) => {
    ...
    const isAvailable = await projectService.existingName(projectObj.project_name);

    if (!isAvailable) {
        throw new Error('Project Name already exists');
    }

    return projectService.create(projectObj);
};
...
const findAll = async (limit = 10, offset = 0, filters = {}) => {
    ...
    const { rows, count} = await projectService.findAll(
        where, limit, offset, 
    )
...
const findByPk = async (id) => {
    ...
    const project = await projectService.findByPk(id);
